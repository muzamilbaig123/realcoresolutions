...
export default function MegaMenuContent({
  ...
  return (
    <div
      className={`grid gap-x-8 py-8 px-4 ${colClass[columns.length] ?? "grid-cols-3"}`}
    >
      {columns.map((col) => (
            ...
            {col.items.map((item) => (
              <Link key={item.id} href={item.href} className="group block">
                <div className="flex items-center gap-1 text-sm font-medium text-gray-800 dark:text-white/80 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-150">
                  ...
                  <ArrowRight
                    size={11}
                    className="opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-150 shrink-0"
                  />
                </div>
